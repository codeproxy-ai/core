...
describe('fallback upstream', () => {
  it('routes to fallback when latest message has an image', async () => {
    let capturedUrl = '';
    const upstream: typeof fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      capturedUrl = typeof input === 'string' ? input : input.toString();
      return new Response(
  ...
  it('stays on primary upstream when latest message has no images', async () => {
    let capturedUrl = '';
    const upstream: typeof fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      capturedUrl = typeof input === 'string' ? input : input.toString();
      return new Response(
  ...
  it('does not fallback when image is only in older messages', async () => {
    let capturedUrl = '';
    const upstream: typeof fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      capturedUrl = typeof input === 'string' ? input : input.toString();
      return new Response(
    ...
    await fetchImpl('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        input: [
          ...
          { type: 'message', role: 'user', content: 'follow-up text' },
        ],
      }),
    });
  ...
  it('uses fallback upstream format when configured', async () => {
    ...
    const upstream: typeof fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      capturedUrl = typeof input === 'string' ? input : input.toString();
      // Return anthropic-format response for fallback
      return new Response(
        JSON.stringify({
    ...
    expect(capturedUrl).toBe('https://api.anthropic.com/v1/messages');
    // With anthropic format, no /chat/completions suffix
