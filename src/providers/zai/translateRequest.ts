...
/** Convert a Responses API request into a ZAI (OpenAI-chat-compatible) request. */
...
function mapTools(tools: ResponsesTool[]): OpenAiChatTool[] {
  ...
  for (const tool of tools) {
    ...
    const tt = tool.type;
    if (tt === 'function') {
      // Normalize flat → wrapped, then strip `strict` (ZAI rejects it).
    ...
    // Drop unknown tool types (parity with reference's retrieval drop).
  }
  return out;
}
